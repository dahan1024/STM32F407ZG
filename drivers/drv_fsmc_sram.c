...
//在指定地址(start_address + Bank1_SRAM3_ADDR)开始,连续写入n个字节.
//buf:字节指针
//start_address:要写入的地址
//len:要写入的字节数
int32_t fsmc_sram_write_buffer(uint32_t start_address, uint8_t *buf, uint32_t len)
{
	if (!len || (start_address + len > FSMC_SRAM_SIZE))
		return RC_INVAL;

	for (; len!=0; len--)
	{
		*(vu8*)(Bank1_SRAM3_ADDR + start_address) = *buf;
		start_address++;
		buf++;
	}
...
//在指定地址((start_address + Bank1_SRAM3_ADDR))开始,连续读出n个字节.
//buf:字节指针
//start_address:要读出的起始地址
//len:要写入的字节数
int32_t fsmc_sram_read_buffer(uint32_t start_address, uint8_t *buf, uint32_t len)
{
	...
	for (; len!=0; len--)
	{
		*buf = *(vu8*)(Bank1_SRAM3_ADDR + start_address);
		start_address++;
		buf++;
	}
